...
const consejos = [
  ...
  { title: "Hidratación", text: "Distribuye el agua a lo largo del día, especialmente alrededor de la actividad física.", icon: Droplets },
...
export default function ClientRecomendacionesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      setData(await orientacionService.obtener(user.clienteId));
    } catch (cause) {
  ...
  return <div>
    <SectionHeader
      ...
      action={<button onClick={() => void load()} disabled={loading} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 disabled:opacity-50"><RefreshCw size={14} className={loading ? "animate-spin" : ""}/>Actualizar</button>}
    />
...
function EvaluationCard({ data }: { data: OrientacionResponse }) {
  ...
  return <Card className="overflow-hidden">
    <div className="border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-emerald-50 p-5">
      ...
      <div className="mt-2 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-bold text-slate-900">{labels[evaluation.clasificacion]}</h2><p className="mt-1 text-xs text-slate-600">Día evaluado: {new Date(`${evaluation.fechaEvaluada}T00:00:00`).toLocaleDateString("es-PE", { dateStyle: "long" })}</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-medium text-slate-600 shadow-sm">Predicción #{evaluation.prediccionId}</span></div>
    </div>
