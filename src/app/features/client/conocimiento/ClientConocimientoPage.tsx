...
export default function ClientConocimientoPage() {
  ...
  useEffect(() => {
    ...
    conocimientoIaService.obtener(user.clienteId, today())
      .then((data) => { setSession(data); setResult(data.resultadoAdaptativo); })
      .catch(() => { setSession(null); setResult(null); })
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">Evaluación diaria de 5 preguntas generada según tu objetivo, tus metas nutricionales y el resultado del modelo predictivo.</p>
      </div>
...
function Summary({ session, result, onBegin, onFeedback }: { session: SesionConocimientoResponse | null; result: ResultadoAdaptativoResponse | null; onBegin: () => void; onFeedback: () => void }) {
  if (!session) return <EmptyState title="Evaluación diaria pendiente" description="Primero realiza el análisis predictivo. Al completarse, el backend generará y guardará una sola evaluación para esa fecha." />;
  ...
  return <div className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
    <Card className="p-6">
      <div className="flex items-center gap-2"><Award size={18} className="text-emerald-600" /><h2 className="font-semibold text-slate-900" style={FONT_HEADING}>Resultado diario</h2></div>
      {result ? <>
...
function Evaluation({ session, result, question, index, selected, setSelected, onNext, onPrevious, onBegin, onSummary, saving }: any) {
  ...
  return <Card className="p-5 sm:p-6">
    <div className="flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-wide text-[#397065]">Evaluación diaria</p><span className="text-xs text-slate-500">Pregunta {index + 1} de {session.preguntasAdaptativas.length} · 2 puntos</span></div>
